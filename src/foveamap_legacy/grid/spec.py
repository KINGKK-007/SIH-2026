...
Examples
--------
>>> from foveamap_legacy.grid.spec import Ring, GridSpec
>>> rings = (
...
def load_spec_from_preset(preset: str) -> GridSpec:
    ...
    # Search for YAML
    candidates = [
        Path("configs/legacy/grids") / f"{preset}.yaml",
        Path(__file__).parent.parent.parent.parent / "configs" / "legacy" / "grids" / f"{preset}.yaml",
    ]
    for c in candidates:
        if c.exists():
