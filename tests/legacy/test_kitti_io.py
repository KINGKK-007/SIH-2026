...
class TestSyntheticScan:
    ...
    def test_z_ground_near_minus_1_73(self) -> None:
        ...
        from foveamap_legacy.io.labels import unpack_kitti_labels
        from foveamap_legacy.io.synthetic import generate_synthetic_scan
    ...
    def test_contains_drivable(self) -> None:
        from foveamap_legacy.io.labels import DRIVABLE, to_superclass, unpack_kitti_labels
        from foveamap_legacy.io.synthetic import generate_synthetic_scan
    ...
    def test_contains_non_drivable_terrain(self) -> None:
        from foveamap_legacy.io.labels import NON_DRIVABLE_TERRAIN, to_superclass, unpack_kitti_labels
        from foveamap_legacy.io.synthetic import generate_synthetic_scan
        pts, raw = generate_synthetic_scan(num_points=5000)
        sem, _ = unpack_kitti_labels(raw)
        sc = to_superclass(sem)
    ...
    def test_contains_static_obstacle(self) -> None:
        from foveamap_legacy.io.labels import STATIC_OBSTACLE, to_superclass, unpack_kitti_labels
        from foveamap_legacy.io.synthetic import generate_synthetic_scan
        pts, raw = generate_synthetic_scan(num_points=5000)
        sem, _ = unpack_kitti_labels(raw)
        sc = to_superclass(sem)
    ...
    def test_contains_dynamic(self) -> None:
        from foveamap_legacy.io.labels import DYNAMIC, to_superclass, unpack_kitti_labels
        from foveamap_legacy.io.synthetic import generate_synthetic_scan
        pts, raw = generate_synthetic_scan(num_points=5000)
        sem, _ = unpack_kitti_labels(raw)
        sc = to_superclass(sem)
    ...
    def test_no_unknown_points(self) -> None:
        ...
        from foveamap_legacy.io.labels import UNKNOWN, to_superclass, unpack_kitti_labels
        from foveamap_legacy.io.synthetic import generate_synthetic_scan
        pts, raw = generate_synthetic_scan(num_points=5000)
        sem, _ = unpack_kitti_labels(raw)
        sc = to_superclass(sem)
